...
## 사용법 A0 — 우리 앱(utlab.kr)에 내장 (`?dc=1`)

라이브/로컬 앱에서 **로그인한 상태로 URL에 `?dc=1`** 을 붙이면 오버레이가 뜹니다.
파일은 `frontend/public/design-commenter.js`로 배포되고, `main.tsx` 로더가
**`?dc=1` + 로그인(JWT)** 조건일 때만 주입합니다(익명 방문자는 안 받음, 소스 경로 미노출 = Selector 기반).

```
utlab.kr/?dc=1            # 랜딩(로그인 후)
utlab.kr/portfolio?dc=1   # 대시보드 각 페이지
```
→ 우하단 패널 → ✎ 편집 → 요소 클릭·코멘트 → 📋 복사 → Claude Code에 붙여넣기.
소스 매핑은 없고(운영 빌드) Selector+텍스트로 잡히며, 정확한 파일:라인은 리포에서 매핑.
`frontend/public/design-commenter.js` 는 `tools/design-commenter/design-commenter.js` 의 배포용 사본(동일 파일, 함께 관리).

## 사용법 A — 내장 브라우저 앱 (로컬 서버, 정적 HTML·URL용)
