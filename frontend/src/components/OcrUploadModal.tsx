...
function ResultRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-center py-1.5 border-b border-zinc-100 dark:border-zinc-800 last:border-0">
      ...
      <span className={`text-xs font-semibold tabular-nums ${value === '-' ? 'text-zinc-400 dark:text-zinc-500' : 'text-zinc-800 dark:text-zinc-200'}`}>{value}</span>
    </div>
  )
}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
          <div>
            <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">스크린샷으로 업데이트</p>
            <p className="text-xs text-zinc-400">{ITEM_LABELS[item]} · {title}</p>
          </div>
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400">
        ...
        <div className="p-4 space-y-3">
          {hint && (
            <p className="text-xs text-zinc-400 dark:text-zinc-500 bg-zinc-50 dark:bg-zinc-800 rounded-lg px-3 py-2">{hint}</p>
          )}

          {/* 업로드 영역 */}
          <div
            className="border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-xl overflow-hidden cursor-pointer hover:border-accent transition-colors"
            onDrop={handleDrop}
          ...
          >
            {preview ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 gap-2">
                ...
                <p className="text-xs text-zinc-400">탭하여 이미지 선택 또는 드래그</p>
                <p className="text-2xs text-zinc-400 dark:text-zinc-500">JPG · PNG · WebP</p>
              </div>
            )}
          </div>
          <input
          ...
          {result && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">인식 결과</p>
              <div className="card-inner px-3 py-1">
                {renderResult(item, result)}
              </div>
              {!hasValues && (
                <p className="text-xs text-zinc-400 text-center">인식된 항목이 없습니다. 더 선명한 이미지를 시도해주세요.</p>
              )}
            </div>
          )}
          ...
          {result && hasValues && (
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-xs text-zinc-500 font-medium"
              >
                다시 찍기
              </button>
              <button
