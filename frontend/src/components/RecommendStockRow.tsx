...
const RecommendStockRow: React.FC<RecommendStockRowProps> = ({ item }) => {
  ...
  return (
    <div className={`flex flex-col gap-1 py-2.5 pl-5 pr-4 ${rowBg} hover:brightness-95 transition-colors border-b border-zinc-50 dark:border-zinc-800/40 last:border-0`}>
      {/* Top row: name + price */}
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {isAI && item.ai_session && (
              <span className="text-2xs px-1.5 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 rounded">
        ...
        <div className="flex items-center gap-3 text-xs tabular-nums flex-shrink-0">
          ...
          {item.news_count > 0 && (
            <span className="text-zinc-400">뉴스 {item.news_count}건</span>
          )}
      ...
      {item.reason && (
        <div className="ml-5 pl-0">
          <p className="text-2xs text-zinc-500 dark:text-zinc-400 leading-relaxed">{item.reason}</p>
        </div>
      )}
      ...
      {item.technical_summary && (
        <div className="ml-5">
          <p className="text-2xs text-zinc-400 dark:text-zinc-500 italic">{item.technical_summary}</p>
        </div>
      )}
      ...
      {item.entry_price != null && (
        <div className="ml-5 flex flex-wrap gap-x-4 gap-y-0.5 text-2xs tabular-nums">
          <span className="text-zinc-400">
            진입 <span className="text-zinc-600 dark:text-zinc-300">{formatPrice(item.entry_range_low)}~{formatPrice(item.entry_range_high)}</span>
          </span>
          <span className="text-zinc-400">
      ...
      {!item.reason && item.latest_news_title && (
        <div className="ml-5">
          <p className="text-2xs text-zinc-400 dark:text-zinc-500 truncate leading-snug">
