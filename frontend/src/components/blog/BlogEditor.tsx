    ...
    <div className="flex flex-col h-full border border-zinc-200 dark:border-zinc-700 rounded-xl overflow-hidden bg-white dark:bg-zinc-900">
      ...
      <BubbleMenu editor={editor} shouldShow={showLinkMenu} tippyOptions={{ duration: 100, appendTo: () => document.body }}>
        <div className="flex items-center gap-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-xl p-1.5">
          ...
          <button onMouseDown={e => { e.preventDefault(); openLinkDialog() }} className="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-500">
            <Link2 size={12} />
          </button>
              ...
              <>
                ...
                <div className="relative">
                  <div className="flex items-center">
                    <button
                      ...
                      title="형광펜"
                      className={`p-1.5 rounded-l transition-colors ${editor.isActive('highlight') ? 'bg-accent/15 text-accent' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
                    >
                      <Highlighter size={14} />
                    </button>
                    <button
                      ...
                      <form onSubmit={applyLink} className="flex gap-1.5 min-w-[280px]">
                        ...
                        {editor.isActive('link') && (
                          <button type="button" onClick={() => { editor.chain().focus().unsetLink().run(); setLinkDialog(false) }}
                            className="px-2 py-1 text-xs bg-zinc-100 dark:bg-zinc-700 text-zinc-500 rounded-lg hover:bg-zinc-200">
                            <Link2Off size={12} />
                          </button>
                        )}
                ...
                <div className="relative">
                  ...
                  {tableDropdown && !inTable && (
                    <div
                      className="absolute top-full left-0 mt-1 z-50 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-2xl p-3"
                            ...
                            return (
                              <button
                                ...
                                className={`w-5 h-5 rounded-sm border transition-colors ${
                                  active ? 'bg-accent/30 border-accent' : 'border-zinc-200 dark:border-zinc-600 hover:border-zinc-300'
                                }`}
                              />
                            )
                          })
      ...
      {mdMode ? (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/10 border-b border-emerald-200 dark:border-emerald-800/40 flex items-center justify-between gap-2 flex-shrink-0">
            ...
            <span className="text-[10px] text-zinc-400 flex-shrink-0">붙여넣기 시 자동 변환</span>
          </div>
          <textarea
            value={mdText}
            onChange={e => setMdText(e.target.value)}
            className="flex-1 p-5 font-mono text-sm text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 outline-none resize-none leading-relaxed"
            placeholder={'# 제목\n\n## 소제목\n\n본문 내용...\n\n- 항목 1\n- 항목 2\n\n**굵게** *기울임* ~~취소선~~\n\n> 인용구\n\n```\n코드 블록\n```'}
            spellCheck={false}
          />
          <div className="px-4 py-1.5 border-t border-zinc-100 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/30 text-right flex-shrink-0">
            <span className="text-[11px] text-zinc-400">{mdText.length.toLocaleString()} 자</span>
          </div>
        </div>
      ) : (
        <>
