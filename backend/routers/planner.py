...
async def planner_chat(
    ...
    def _nps_months_to(target_year: int) -> int:
        return max(0, (target_year - ctx.nps_join_year) * 12 - (ctx.nps_join_month - 1))

    nps_months_at_retire = _nps_months_to(retire_year_val)
    nps_months_at_60 = _nps_months_to(nps_60_year)

    def _nps_period_factor(months: int) -> float:
        """국민연금 가입기간 급여 계수: 20년(240개월) 미만이면 비례, 이상이면 가산"""
        if months >= 240:
            return 1.0 + 0.05 * (months - 240) / 12
        return months / 240

    P_retire = _nps_period_factor(nps_months_at_retire)
    P_60 = _nps_period_factor(nps_months_at_60)
    ...
    def calc_full_snap(age: int, isa_period_years: int, hp_monthly_만: int = 0) -> int:
        """특정 나이·ISA 기간의 월 수령액(만원) — 모든 수입/지출 포함.
        ...
        _birth_ym = lambda a: (1983 + a) * 100 + 4  # 4월생 기준
    ...
    def calc_snapshot_monthly(age: int, isa_monthly_gross: int) -> dict:
    ...
    hp_at_retire_net_만 = round(hp_net_at_retire / 10000)
    ...
    def build_snap_table(isa_period: int, hp_apply_age: int, hp_월: int) -> str:
        ...
        for age in _snap_ages:
            ...
            val = calc_full_snap(age, isa_period, hp)
    ...
    def calc_full_snap_pp10(age: int, isa_period_years: int, hp_monthly_만: int = 0) -> int:
        total = 0
        isa_end_age = ctx.retirement_age + isa_period_years
        if isa_period_years > 0 and age < isa_end_age:
        ...
        if age < ctx.mortgage_paid_off_age:
            total -= round(mortgage_monthly_만_val)
        _birth_ym2 = lambda a: (1983 + a) * 100 + 4
        for _pp in ctx.private_pensions:
            _c = _pp.get('monthly_contrib', 0)
            _po = _pp.get('paid_off_ym') or ''
            if _c > 0 and _po:
    ...
    def build_snap_table_pp10(isa_period: int, hp_apply_age: int, hp_월: int) -> str:
        ...
        for age in _snap_ages:
            hp = hp_월 if age >= hp_apply_age else 0
            val = calc_full_snap_pp10(age, isa_period, hp)
            rows.append(f"  {age}세: {val}만원")
        return "\n".join(rows)
    ...
    _pp_key_ages = [ctx.retirement_age, min(ctx.retirement_age + 1, 59),
                    59, 60, 65, ctx.mortgage_paid_off_age, ctx.nps_receipt_age, 70, 75, 80]
    ...
    snap_retire_no_isa = calc_snapshot_monthly(ctx.retirement_age, 0)
    snap_nps_no_isa = calc_snapshot_monthly(ctx.nps_receipt_age, 0)
    ...
    if result is None:
        raise HTTPException(503, "AI API 응답 없음. 잠시 후 다시 시도해주세요.")
    ...
    try:
        ...
        raise HTTPException(500, f"AI 응답 파싱 실패: {str(e)[:80]}")
